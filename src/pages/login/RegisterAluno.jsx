// src/pages/login/RegisterAluno.jsx
...
export default function RegisterAluno(){
  ...
  async function handleSubmit(e){
    e.preventDefault();
    try{
      await registerUser({ name, email, password, role: "aluno" });
      navigate("/login");
    }catch(err){
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white p-6 rounded shadow">
        <h2 className="text-2xl font-bold mb-4">Cadastro - Aluno</h2>
        <form onSubmit={handleSubmit}>
          ...
          <button className="w-full bg-green-600 text-white p-2 rounded">Cadastrar</button>
        </form>
      </div>
    </div>
  );
